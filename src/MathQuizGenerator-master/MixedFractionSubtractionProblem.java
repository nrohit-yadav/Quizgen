...
public class MixedFractionSubtractionProblem implements IAlgebraProblemFraction{

	private MixedFraction f1 = new MixedFraction();
	private MixedFraction f2 = new MixedFraction();
	
	
	public MixedFractionSubtractionProblem(){
	...
	public ProblemAndAnswerFraction generateProblem(int category){
		...
		Fraction fdiv = MixedFraction.add(f1, f2);
	...
	private static void printHTMLHeader() {
		...
		htmlString += "<title>Mixed Fraction Subtraction Problems</title>";
		htmlString += "</head>";
		htmlString += "<body>";
	...
	public static void printProblemsWithMultipleChoiceAnswers(int count){
		...
		for(int k=0; k<count; k++){
			MixedFractionSubtractionProblem s = new MixedFractionSubtractionProblem();
