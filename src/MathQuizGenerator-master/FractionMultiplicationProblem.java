...
public class FractionMultiplicationProblem implements IAlgebraProblemFraction{

	private Fraction f1 = new Fraction();
	private Fraction f2 = new Fraction();
	
	
	public FractionMultiplicationProblem(){
	...
	public ProblemAndAnswerFraction generateProblem(int category){
		...
		Fraction fdiv = Fraction.multiply(f1, f2);
	...
	private static void printHTMLHeader() {
		...
		htmlString += "<title>Fraction Multiplication Problems</title>";
		htmlString += "</head>";
		htmlString += "<body>";
	...
	public static void printProblemsWithMultipleChoiceAnswers(int count){
		...
		for(int k=0; k<count; k++){
			FractionMultiplicationProblem s = new FractionMultiplicationProblem();
