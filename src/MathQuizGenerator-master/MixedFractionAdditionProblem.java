...
public class MixedFractionAdditionProblem implements IAlgebraProblemFraction{

	private MixedFraction f1 = new MixedFraction();
	private MixedFraction f2 = new MixedFraction();
	
	
	public MixedFractionAdditionProblem(){
	...
	private static void printHTMLHeader() {
		...
		htmlString += "<title>Mixed Fraction Addition Problems</title>";
		htmlString += "</head>";
		htmlString += "<body>";
	...
	public static void printProblemsWithMultipleChoiceAnswers(int count){
		...
		for(int k=0; k<count; k++){
			MixedFractionAdditionProblem s = new MixedFractionAdditionProblem();
