...
public class FractionSubtractionProblem implements IAlgebraProblemFraction{

	private Fraction f1 = new Fraction();
	private Fraction f2 = new Fraction();
	
	
	public FractionSubtractionProblem(){
	...
	public ProblemAndAnswerFraction generateProblem(int category){
		...
		Fraction fdiv = Fraction.subtract(f1, f2);
	...
	private static void printHTMLHeader() {
		...
		htmlString += "<title>Fraction Subtraction Problems</title>";
		htmlString += "</head>";
		htmlString += "<body>";
	...
	public static void printProblemsWithMultipleChoiceAnswers(int count){
		...
		for(int k=0; k<count; k++){
			FractionSubtractionProblem s = new FractionSubtractionProblem();
