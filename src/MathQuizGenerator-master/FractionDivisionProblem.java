...
public class FractionDivisionProblem implements IAlgebraProblemFraction{

	private Fraction f1 = new Fraction();
	private Fraction f2 = new Fraction();
	
	
	public FractionDivisionProblem(){
	...
	public ProblemAndAnswerFraction generateProblem(int category){
		...
		Fraction fdiv = Fraction.divide(f1, f2);
	...
	private static void printHTMLHeader() {
		...
		htmlString += "<title>Fraction Division Problems</title>";
		htmlString += "</head>";
		htmlString += "<body>";
	...
	public static void printProblemsWithMultipleChoiceAnswers(int count){
		...
		for(int k=0; k<count; k++){
			FractionDivisionProblem s = new FractionDivisionProblem();
